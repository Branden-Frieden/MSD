...
int main()
{
   ...
   */
    ...
    // the min of v1 is -1 so this should return false
    if(  FindMin(v1) != -1){
        ErrorExit("FindMin() - test 1");
    }
    
    // the min of v2 is 1 so this should return false
    if(  FindMin(v2) != 1){
        ErrorExit("FindMin() - test 2");
    }
    
    // the initialized value for min is INT_MAX so a empty array should return it
    if(  FindMin(empty) != INT_MAX){
        ErrorExit("FindMin() - test 3");
    }
    
    // the max of v1 is 5 so this should return false
    if( FindMax(v1) != 5){
        ErrorExit("FindMax() - test 1");
    }
    
    // the max of v2 is 3 so this should return false
    if( FindMax(v2) != 3){
        ErrorExit("FindMax() - test 2");
    }
    
    // the initialized value for max is INT_MIN so a empty array should return it
    if( FindMax(empty) != INT_MIN){
        ErrorExit("FindMax() - test 3");
    }
    
    // the average of v1 should return 1 so this should return false
    if( Average(v1) != 1){
        ErrorExit("Average() - test 1");
    }
    
    // the average of v2 is 2 so this should return false
    if( Average(v2) != 2){
        ErrorExit("Average() - test 2");
    }
    
    // total is initialized to 0 so an empty vector should return 0 so this will be false
    if( Average(empty) != 0){
    ...
    // and empty vector should return false to avoid errors
    if( IsSorted(empty)){
