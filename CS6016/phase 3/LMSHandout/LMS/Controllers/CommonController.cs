...
namespace LMS.Controllers
{
    public class CommonController : Controller
    {
        ...
        public IActionResult GetSubmissionText(string subject, int num, string season, int year, string category, string asgname, string uid)
        {
            ...
            var query = from s in db.Submissions where
                        s.Student == uid &&
        ...
        public IActionResult GetUser(string uid)
        {
            ...
            var students = from u in db.Students where u.UId == uid select new { fname = u.FName, lname = u.LName };
            var professors = from u in db.Professors where u.UId == uid select new { fname = u.FName, lname = u.LName };
            var admins = from u in db.Administrators where u.UId == uid select new { fname = u.FName, lname = u.LName };

            if (students.Any())
            {
                return Json(students.ToArray());
            }
            else if (professors.Any())
            {
                return Json(professors.ToArray());
            }
            else if (admins.Any())
            {
                return Json(admins.ToArray());
            }
