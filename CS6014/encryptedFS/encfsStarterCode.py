            ...
            try:
                with open(full_path, "rb") as file:
                    ...
                    if os.path.getsize(full_path) == 0:
                        self.fdCounter += 1
                        self.openFiles[self.fdCounter] = b''
                    ...
                    print('initialized kdf')
                    print(bytes(self.password, 'utf-8'))
                    key = base64.urlsafe_b64encode(kdf.derive(bytes(self.password, 'utf-8')))
                    print('created key')
                    f = Fernet(key)
                    print('created fernet')
                    decrypted = f.decrypt(file.read())
                    print('decrypted')
                    print(self.fdCounter)
                    self.fdCounter += 1
                    print(self.fdCounter)
                    self.openFiles[self.fdCounter] = decrypted
